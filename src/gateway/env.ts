...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  // OpenRouter takes highest precedence (uses OpenAI-compatible API)
  if (env.OPENROUTER_API_KEY) {
    envVars.OPENROUTER_API_KEY = env.OPENROUTER_API_KEY;
    envVars.OPENAI_API_KEY = env.OPENROUTER_API_KEY; // OpenRouter uses OpenAI-compatible API
    envVars.OPENAI_BASE_URL = 'https://openrouter.ai/api/v1';
    envVars.AI_GATEWAY_BASE_URL = 'https://openrouter.ai/api/v1/openrouter'; // Signal to use OpenRouter config
  } else {
